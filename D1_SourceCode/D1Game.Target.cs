...
public class D1GameTarget : TargetRules
{
	public D1GameTarget(TargetInfo Target) : base(Target)
	{
		...
		D1GameTarget.ApplySharedD1TargetSettings(this);
	}
		...
		else
		{
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Logger.LogWarning("D1GameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}
	}

...
{
    // Register delegate for future game instances
    ...
    // Immediately register for any existing world contexts
    if (GEngine)
    {
        const TIndirectArray<FWorldContext>& WorldContexts = GEngine->GetWorldContexts();
        for (const FWorldContext& WorldContext : WorldContexts)
        {
            RegisterInputContextMappingsForGameInstance(WorldContext.OwningGameInstance);
        }
    }
}
...
void UGameFeatureAction_AddInputContextMapping::RegisterInputContextMappingsForGameInstance(UGameInstance* GameInstance)
{
    ...
    // Bind to local player added/removed events
    ...
    // Register for existing local players
    for (ULocalPlayer* LP : GameInstance->GetLocalPlayers())
    {
        RegisterInputMappingContextsForLocalPlayer(LP);
    }
}
...
void UGameFeatureAction_AddInputContextMapping::RegisterInputMappingContextsForLocalPlayer(ULocalPlayer* LocalPlayer)
{
	if (ensure(LocalPlayer))
	{
		...
		if (UEnhancedInputLocalPlayerSubsystem* EISubsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
		{
			...
			{
                for (const FInputMappingContextAndPriority& Entry : this->InputMappings)
				{
					...
					if (UInputMappingContext* IMC = Cast<UInputMappingContext>(AssetManager.GetAssetRegistry().GetAssetByObjectPath(Entry.InputMapping.ToSoftObjectPath()).GetAsset()))
					{
...
void UGameFeatureAction_AddInputContextMapping::UnregisterInputMappingContexts()
{
    ...
    if (GEngine)
    {
        ...
        for (const FWorldContext& WorldContext : WorldContexts)
        {
            UnregisterInputContextMappingsForGameInstance(WorldContext.OwningGameInstance);
        }
    }
}
...
void UGameFeatureAction_AddInputContextMapping::UnregisterInputContextMappingsForGameInstance(UGameInstance* GameInstance)
{
	if (GameInstance != nullptr)
	{
        ...
        for (ULocalPlayer* LP : GameInstance->GetLocalPlayers())
        {
            UnregisterInputMappingContextsForLocalPlayer(LP);
        }
	}
}
